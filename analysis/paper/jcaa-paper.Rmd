...
In this paper we investigate recent changes in the way computational archaeology is being done. Revolutions or paradigm-shifts in archaeology have been discussed to happen following theoretical discurses[@clark_1993; @härke_2002], but there has been
an argument in philosophy of science whether revolutions in science are either ideas-driven or tool-driven [@galison_1997; @dyson_2000]. In this light we explore how many scientific disciplines have started to embrace methods to improve the reproducibility of their research and whether this change is part of a paradigm-shift. We focus on one set of tools for this change which is based on writing scripts in free and open source programming languages and review bibliometric evidence for this change in the sciences generally and in archaeology in particular. We evaluate how computational approaches to improving reproducibility and transparency in archaeology are mediated and transformed by digital approaches and test if the adoption of these new tools may be considered a revolution or some other process. 

[need some more in here]
...
The question of predicting or generating change in scientific practice via adoption of new tools is especially intruiging, and in this section we explore how a change in ecology might be relevant for understanding the future of archaeology. 
...
A pragamtic issue relating to wide-spread changes in scientific practice is what the incentives are for members of the research community that might not be motivated by abstract basic principles of the science method. If code sharing has some concrete career advantages then we might expect it to be more likely to be part of a tool-drive revolution.  @Vandewalle2012 analysed citation rates for 645 articles published in _IEEE Transactions on Image Processing_ during 2004–2006. He found statistically significant differences between the median number of citations for papers that have code available and papers that do not have code available. In Figure \@ref(fig:citation-advantage) we conducted a similar anlysis for archaeology articles. We see that archaeological articles that cite R are consistently more highly cited themselves than articles that do not cite R. 
...
In light of the discussion in the philosophy of science, whether paradigm-shifts are tool- or idea-driven, we offer the hypothesis that a tool-driven change of paradigms is underway in the biological sciences and, more subtly, in archaeology. The tool which drives this change is the use of scripting languages in computational approaches of these disciplines. We could show that the use of R is on the rise among archaeologists using natural and physical science methods, and has concrete advantages for authors that publish papers that cite R as they tend to be referenced more often. This also is an indicator for the increase in popularity of R. 

The use of R or any other scripting languages enables a paradigm-shift, as it offers the possibility to share one's code. The sharing of code has been identified in other disciplines as an amplifier of methodological change [@touchon_and_mccoy_2016]. It also enables researchers to not just reproduce the work of others, but to also evaluate them according to the scientific method. Thus, the sharing of code may be at the root of a tool-driven paradigm-change as desribed in the work of @Galison [-@Galison_1997]. This change does not take place revolutionary-quick and does not replace older ideas as such (as has been a critique of Kuhn's idea-driven concept of paradigm shift by others [@meltzer_1979, @trigger_2006]). Rather, this enables the archaologist to return to the tenet of the scientific method which demands the reproducibility of results. We do not want to claim archaeologists did not want to create reproducible research before the advance of scientific scripting languages. There is a longstanding tradition of publishing catalogues, lists and maps showing the data used for analyses. In the digital age, though, many archaeological tasks have shifted from the descriptive and qualitative aspects to quantitative and computational analyses. This shift also mandates a change in documentation of analyses to enable other researchers to reproduce published results. Modern archaeology has often been lacking in this aspect, as the research has been undertaken using point-and-click software, which complicates the reproducibility of analyses. As we could show, the advance of scripting languages takes place mostly in the part of archaeology that is in close contact with biology (zooarchaeology) or focuses on scientific analyses.

We do not believe, that speaking with Kuhn's view on paradigm-change, a change in the view point of researchers seems to be happening: Data-and method sharing behaviour has been around in archaeology before. What we can see, though, is that the need for methods to share research in a way that is reproducible for others has been acknowledged and that archaeologists have looked for tools to make this possible. Using  scripting languages and data repositories is a solution, that enjoys increasing popularity.

We acknowledge, this is not an easy task. Learning a scripting language is time-intensive and writing code so that one is comfortable in sharing it even more so. 

[some more info from your case study maybe?]

To make it easier for archaeologists and researchers in other fields to use R for research and publication, we describe the R package rrtools ('reproducible research tools'). This package aims to simplify many of the steps needed to write reproducible research, and to guide users to best practices with minimal effort. The rrtools package contains functions that create a file structure according to fundamental principles of organising files for research. First, we keep the data separate from the methods. This means keep the code in a separate directory to the data. Second, we, keep the raw data seprate from data that is created or derived during the analysis.These two basic principles make it easy to stay organised during a complicated project, and make it easier for a reader to navigate their way through the compendium of files that are shared with the publication. 

The rrtools package contains several functions to enable a researcher to quickly set up a compendium suitable for writing reproducible research in R. Each of these steps reflects a best practice that has been previously articulated in other feilds. We will focus here on the first five, as these are most relevant to most researchers:
...
Using rrtools or its addin for Rstudio [cite Clemens here] enables the researcher to easily set up a digital work place that enables reproducible research. [do we name people here, who've already used it?]

# Conclusion

Starting with the discussion of Kuhn's influential work on paradigm-shifts in science [citation needed], we offer a second explanation on how pardigm-shifts develop in science with the help of Galison [-@galison_1997], who focusees on the material culture of a science and its tools. A change in the paradigms of a discipline may also only occur, if tools to implement this change are available. The change we are aiming for is the move to a reproducible computational archaeology, that adheres to the scientific method. The tool which we have been discussing here is R, as an example for scientific scripting languages. 

We have shown a bibliometric analysis that reveals substantial change in the use of the R programming languages in ecology  We saw that ecological and evolutionary sciences have strongly embraced R in their publications. Archaeologists have more recently taken up R, but to a much lesser degree, so far, than what we see in the other fields, most commonly in more scientific aspects of archaeology. We also identified a small sub-set of archaeology papers that use R to make their work fully reproducible and could identify an advantage of citing R for researchers as it leads to being referenced more often. 

There is a lot of potential for other archaeologists to make their work reproducible, and we think this is a much needed paradigm-shift in computational archaeology - linking the digital and computational times to  older practices of sharing data and methods. We therefore presented our freely available R package, rrtools, which is designed to make it easy for researchers in archaeology and other fields to make their research reproducible. We hope this will help to realise some of the potential to make computational archaeological research more open and reproducible and in this way encourage researchers to adhere to the scientific method.       
